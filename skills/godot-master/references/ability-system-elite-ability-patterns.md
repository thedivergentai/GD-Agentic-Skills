...
| Strategy | When | Notes |
|----------|------|-------|
| Per-ability | Default | Independent timers per `ability_id` — see [ability_manager.gd](../scripts/ability_system_ability_manager.gd) |
...
| Charge recharge | Mobility spells | [charge_ability.gd](../scripts/ability_system_charge_ability.gd) — tick in `_physics_process` |

## Status effects — duplicate trap

> **CAUTION:** Applying a status template without `duplicate(true)` mutates the shared `.tres` for every character. Always append `effect_template.duplicate(true)` in [status_effect_manager.gd](../scripts/ability_system_status_effect_manager.gd).
...
[combo_tracker.gd](../scripts/ability_system_combo_tracker.gd) — windowed sequences; finisher abilities stay normal `AbilityResource` entries granted only via `combo_triggered` signal.

## Skill tree architecture

- **Progression data:** [skill_node.gd](../scripts/ability_system_skill_node.gd) + [skill_tree_manager.gd](../scripts/ability_system_skill_tree_manager.gd) — duplicate templates per player save.
- **Combat casts:** Scene-scoped [ability_manager.gd](../scripts/ability_system_ability_manager.gd) on the caster — skill tree grants Resources; never `get_node("/root/AbilityManager")` for combat.
- **Design-time graphs:** `@tool` GraphEdit visualizer for prerequisite auditing (optional editor script).

## Networking

[ability_caster_network.gd](../scripts/ability_system_ability_caster_network.gd) — client predicts VFX/audio immediately; server validates costs and emits confirm/cancel RPCs. Server wins on mismatch.
