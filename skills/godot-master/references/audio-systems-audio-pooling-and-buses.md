...
**Fix:** Pre-allocate a round-robin pool — [audio_voice_pool_manager.gd](../scripts/audio_systems_audio_voice_pool_manager.gd). Cap identical SFX with [audio_voice_limiter_manager.gd](../scripts/audio_systems_audio_voice_limiter_manager.gd) to avoid constructive clipping when 50 explosions stack.
...
- `set_bus_volume_db()` is logarithmic; UI sliders need `linear_to_db()` — [audio_linear_volume_interpolator.gd](../scripts/audio_systems_audio_linear_volume_interpolator.gd).
