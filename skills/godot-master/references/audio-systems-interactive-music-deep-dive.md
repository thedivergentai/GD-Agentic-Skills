...
State-machine of clips with transition rules that snap to beat/bar boundaries (explore → combat). Wire via [interactive_music_graph.gd](../scripts/audio_systems_interactive_music_graph.gd).

## Vertical layering (`AudioStreamSynchronized`)

Multiple stems stay sample-locked. Fade stems with `set_sync_stream_volume(index, db)` over 1–2s Tweens based on intensity. Wire via [audio_interactive_music_manager.gd](../scripts/audio_systems_audio_interactive_music_manager.gd).

## Rules

- Never hard-cut music; always crossfade or interactive transition.
- Keep Music on its own bus; duck under Dialogue with [audio_bus_ducker_logic.gd](../scripts/audio_systems_audio_bus_ducker_logic.gd).
