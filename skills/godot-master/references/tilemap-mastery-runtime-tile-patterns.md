...
Cache hot reads — [fast_metadata_cache.gd](../scripts/tilemap_mastery_fast_metadata_cache.gd).

## Destructible / highlight

See [destructible_tile_logic.gd](../scripts/tilemap_mastery_destructible_tile_logic.gd). After erase: refresh nav ([nav_mesh_teleport_fix.gd](../scripts/tilemap_mastery_nav_mesh_teleport_fix.gd)).

## Chunking

`CHUNK_SIZE` grids as child TileMapLayers; load/unload by player chunk — [tilemap_chunking.gd](../scripts/tilemap_mastery_tilemap_chunking.gd).
