...
Canonical samples: [physics_queries.gd](../scripts/physics_queries.gd), [raycast_vision_stack.gd](../scripts/raycast_vision_stack.gd), [shapecast_aoe.gd](../scripts/shapecast_aoe.gd).

## Performance

- Disable optional raycasts when idle; enable → `force_raycast_update()` → read → disable.
- 1000+ bullets: **MANDATORY** [physics_server_swarm.gd](../scripts/physics_server_swarm.gd) — free RIDs manually (not GC'd).
- High-refresh jitter: [jitter_interpolation_fix.gd](../scripts/jitter_interpolation_fix.gd).

## Elite: compound RID bodies

Multiple shapes on one `PhysicsServer2D` body RID — [compound_body_sync.gd](../scripts/compound_body_sync.gd).

## Debug draw

Contact normals/points from `KinematicCollision2D` — [collision_visual_debugger.gd](../scripts/collision_visual_debugger.gd).
