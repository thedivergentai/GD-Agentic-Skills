...
> **CAUTION:** Runtime HP/mana on a shared `.tres` mutates the asset on disk for every instance. Use `duplicate()` or `resource_local_to_scene` — see [resource_local_to_scene.gd](../scripts/resource_local_to_scene.gd).
...
Use when data must **not** persist — see [data_factory_resource.gd](../scripts/data_factory_resource.gd) for hybrid factories.
...
See [resource_preloading_strategy.gd](../scripts/resource_preloading_strategy.gd).
