...
> **MANDATORY** for JSON-RPC bridges, PWA lifecycle, and host integration beyond bundled scripts. **NEVER** use raw `JavaScriptBridge.eval("localStorage.setItem('%s')" % data)` — use [web_local_storage_wrapper.gd](../scripts/web_local_storage_wrapper.gd).
...
**MANDATORY** [web_json_rpc_bridge.gd](../scripts/web_json_rpc_bridge.gd) for bidirectional browser messaging.
...
Use `get_interface("localStorage")` via [web_local_storage_wrapper.gd](../scripts/web_local_storage_wrapper.gd).
