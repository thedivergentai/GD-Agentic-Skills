...
2. **Score against the bands, not a single metric.** A candidate's score = distance of every style's win rate from its target band center, weighted, plus penalties for: star-histogram degeneracy, downtime dominance, leak concentration on one enemy kind, and economy drift (clear-time change alters coins/minute).
3. **Simulate all styles for every candidate** — a candidate that fixes `average` but drops `afk` to 0% fails.
...
`tune_mode_progression(...)` follows the same pattern for mode curves (e.g. deadline timer per level: `target_time = base + level_id * step` — search `base`/`step` so each level's mode win rates stay in band).
...
2. **Respect the influence graph.** A new level picks its enemy pool from what's unlocked, introduces exactly one new element, and inherits curve coefficients interpolated between neighbors.
3. **Validate by simulation** (`LevelValidation`): run the full style matrix on the candidate; iterate (bounded retries with adjusted parameters) until all bands pass; report the final aggregates alongside the candidate.
...
2. `simulate --runs 1000` full matrix with the standard seed — diff against the saved JSON snapshot.
3. Triage every cell that left its band; classify: intended (the change) vs collateral (shared constants, curve interactions).
4. For collateral damage: `bruteforce` the affected level's local parameters to bring it back in band **without** reverting the new content.
5. `mode --runs 500` for all modes + `career` for casual & average — verify economy and grind targets still hold.
6. Save the new snapshot. Summarize: what changed, what was retuned, before/after win rates per cell.
...
2. `gen-level` with those targets; review the candidate's aggregates AND texture metrics (leaks by kind, downtime, time-to-first-fault) — numbers in band with degenerate texture (all threat from one enemy) still need manual pool/weight edits.
...
4. `career` to confirm progression pacing with the new level inserted.
